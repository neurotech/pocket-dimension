...
exports.handler = (event, context, callback) => {
  ...
  auth.validateSessionToken(event, (error, user) => {
    console.warn("VALIDATE WAS CALLED");
    if (error) {
      var errorResponse = {
        isBase64Encoded: false,
        statusCode: 500,
    ...
    if (event.path === "/items/all") {
      if (event.httpMethod === "GET") {
        getItems(user.userId, false, (error, results) => {
          if (error) {
            var errorResponse = {
              isBase64Encoded: false,
              statusCode: 500,
              headers: {},
              body: "ERROR GETTING ITEMS :" + JSON.stringify(error),
            };
            return callback(errorResponse);
          }
      ...
      console.warn("VALIDATE WAS NOT CALLED");
    }
  });
};
